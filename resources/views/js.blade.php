    ...
    function gameList(data){
        ...
        Object.entries(data.games).forEach((value, key) => {
            ...
            $(`.game_list[data-provider=${data.provider}]`).append(`
                <a href='javascript:void(0)' class="w-32 md:w-1/3 launch_game d-block p-2 my-2" data-game_id="${value[1].providerId}" data-game_link="${value[1].link}">
                    <img src="{{asset("images")}}/gamesImg/${imgSrc}" alt="${value[1].title}" srcset="" class="w-[120px] rounded-lg max-h-[120px]" onerror="this.onerror=null; $(this).parent().remove()">
                </a>
            `);
